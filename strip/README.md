...
- **双击顶部拖拽带 = 吸附**：贴回当前所在那块屏幕的底边、铺满该屏宽度，**高度保持不变**
- 位置和尺寸自动记住，下次启动原样恢复
...
顶部那 26pt 拖拽带双击一次，窗口回到**它当前所在屏幕**的底边、铺满该屏宽度，高度原样不动 ——
这是真标题栏用来 zoom 的手势，这个窗口没有标题栏，就拿它做吸附。

选「当前屏幕」而不是「竖屏」：双击是个就地手势，把窗口传送回另一块屏幕会很意外。
判定方式是跟哪块屏幕的重叠面积最大（`currentScreen()`）。

用的是 `frame` 不是 `visibleFrame`，和首次启动的落点口径一致 —— 所以主屏底部有 Dock 时会盖在 Dock 上
（面板本来就是 `.statusBar` 层级，压在 Dock 之上）。

高度不碰。宽度和位置是「摆哪儿」的问题，随时可以一键复位；高度是「要看几行」的偏好，
把它一并重置掉等于每次吸附都要重新拉高。
